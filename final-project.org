...
#+PROPERTY: header-args:sqlite+ :eval never :tangle final-project.sqlite
...
#+NAME: read-rate-data
#+HEADERS: :var sql_results_file="rate-data.csv"
#+BEGIN_SRC R 
  library(readr)
  library(dplyr)
...
#+BEGIN_SRC R :file bcbsfl-feffects-check.png
...
#+RESULTS:
[[file:bcbsfl-feffects-check.png]]
...
#+HEADERS: :var rating_area_file="govt-rating-areas.csv"
#+BEGIN_SRC R 
  library(reshape2)
...
#+HEADERS: :results replace graphics :rownames no :colnames no :tangle final-project-graphics.R
#+BEGIN_SRC R :file sanity-check.png
...
#+HEADERS: :results replace graphics :rownames no :colnames no :tangle final-project-graphics.R
#+BEGIN_SRC R :file united-outlier-1.png
...
#+HEADERS: :results replace graphics :rownames no :colnames no :tangle final-project-graphics.R
#+BEGIN_SRC R :file united-outlier-2.png
...
Surprisingly to this author, the data tells the same story for BCBSFL.  When we fit a lasso using similar [[final-project-united-lasso][code]], /mutatis mutandis/, we see the following regression terms.
...
#+NAME: bcbsfl-map
#+HEADERS: :results replace graphics :rownames no :colnames no :tangle final-project-graphics.R
#+BEGIN_SRC R :file bcbsfl-map.png
  library(maps)
...
This code to fit a cross-validated lasso regression was given [[final-project-united-lasso][above]].  This time I’ve also included the lines to massage the output for presentation.
...
This last snippet I’m simultaneously proud and ashamed to say I wrote.  One long pipeline to calculate the lasso of the normalized data after removing Union County’s cancer incidence rates.  A big shout-out to the [[https://github.com/Hong-Revo/glmnetUtils][glmnetUtils]] package for fitting ~cv.glmnet~ in one line.  The final coefficient matrix is empty.
